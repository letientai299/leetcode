...
// `n`, where `dist[i]` is the **initial distance** in kilometers of the `ith`
...
// lose, or_ `n` _if you can eliminate all the monsters before they reach the
// city._
//
// **Example 1:**
//
...
func eliminateMaximum(dist []int, speed []int) int {
	for i, d := range dist {
		dist[i] = (d + speed[i] - 1) / speed[i]
	}
	sort.Ints(dist)
	i := 0
	for i < len(dist) && i < dist[i] {
		i++
	}
