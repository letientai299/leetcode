...
// both have the same available memory). If neither stick has at least `i` bits
// of available memory, the program **crashes**.
//
// Return _an array containing_`[crashTime, memory1crash, memory2crash]` _,
// where_ `crashTime` _is the time (in seconds) when the program crashed and_
// `memory1crash` _and_ `memory2crash` _are the available bits of memory in the
// first and second sticks respectively_.
//
// **Example 1:**
//
// ```
