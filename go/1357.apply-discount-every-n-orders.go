...
// `products` and `prices`, where the `ith` product has an ID of `products[i]`
...
// Their subtotal is calculated as the sum of each `amount[j] * (price of the
// jth product)`.
//
...
// subtotal. More formally, if their subtotal is `bill`, then they would
...
// the bill with the discount applied (if any). Answers within `10-5` of the
...
func Constructor(n int, discount int, products []int, prices []int) Cashier {
	...
	return Cashier{
		prices:   m,
		n:        n,
		discount: 1 - float64(discount)/100,
		cur:      0,
	}
}
