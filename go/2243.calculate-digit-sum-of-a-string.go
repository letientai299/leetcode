...
//
// 1. **Divide** `s` into **consecutive groups** of size `k` such that the first
// `k` characters are in the first group, the next `k` characters are in the
...
// its digits. For example, `"346"` is replaced with `"13"` because `3 + 4 + 6 =
// 13`.
// 3. **Merge** consecutive groups together to form a new string. If the length
