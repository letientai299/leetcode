...
// at time `t`. Votes cast at time `t` will count towards our query. In the case
// of a tie, the most recent vote (among tied candidates) wins.
//
...
func Constructor911(people []int, times []int) TopVotedCandidate {
	counts := make(map[int]int, len(people))
	best := 0
	h := 0
...
func (tvc *TopVotedCandidate) Q(t int) int {
	...
	if i >= len(tvc.times) || tvc.times[i] > t {
		i--
	}
	return tvc.tops[i]
}
