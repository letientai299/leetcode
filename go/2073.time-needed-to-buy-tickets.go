...
//
// Return _the **time taken** for the person at position_ `k` **_(0-indexed)_**
// _to finish buying tickets_.
//
// **Example 1:**
//
// ```
...
func timeRequiredToBuy(tickets []int, k int) int {
	...
	if tickets[k] == min {
		return (min-1)*len(tickets) + k + 1
	}

	i := 0
	newK := k
	for j, n := range tickets {
		if n == min {
			if j < k {
				newK--
			}
	...
	return timeRequiredToBuy(tickets[:i], newK) + min*len(tickets)
}
