...
// integer array that contains the answers of the `ith` student (
// **0-indexed**). The answers of the mentors are represented by a 2D integer
// array `mentors` where `mentors[j]` is an integer array that contains the
// answers of the `jth` mentor ( **0-indexed**).
//
...
func maxCompatibilitySum(students [][]int, mentors [][]int) int {
	...
	for i, student := range students {
		...
		for q, v := range student {
			for j, mentor := range mentors {
				if mentor[q] == v {
					scores[i][j]++
				}
			}
