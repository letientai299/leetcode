...
// curOrder)`, which given a person `x` and the inheritance order so far,
...
// 2. Calling `Successor(king, curOrder)` will return Alice, so we append to
// `curOrder` to get `["king", "Alice"]`.
// 3. Calling `Successor(Alice, curOrder)` will return Jack, so we append to
...
// 5. Calling `Successor(Bob, curOrder)` will return `null`. Thus the order of
...
func (n *TINode) add(name string) *TINode {
	node := &TINode{
		name: name,
	}
...
func (ti *ThroneInheritance) Birth(parentName string, childName string) {
	node := ti.m[parentName].add(childName)
	ti.m[childName] = node
}
