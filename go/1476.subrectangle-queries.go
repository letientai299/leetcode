...
// This is not the fastest, but there's no elegant and easy to code solution
// for this problem. The fastest is actually 2 for-loop for UpdateSubrectangle
...
func (rec *SubrectangleQueries) UpdateSubrectangle(row1 int, col1 int, row2 int, col2 int, newValue int) {
	i := 0
	for _, lay := range rec.layers {
		r1, c1, r2, c2 := lay[0], lay[1], lay[2], lay[3]
		contained := row1 <= r1 && r2 <= row2 && col1 <= c1 && c2 <= col2
		if !contained {
			rec.layers[i] = lay
			i++
		}
	...
	rec.layers = append(rec.layers[:i], []int{row1, col1, row2, col2, newValue})
}

func (rec *SubrectangleQueries) GetValue(row int, col int) int {
	for i := len(rec.layers) - 1; i >= 0; i-- {
		lay := rec.layers[i]
		r1, c1, r2, c2, v := lay[0], lay[1], lay[2], lay[3], lay[4]
		if r1 <= row && row <= r2 && c1 <= col && col <= c2 {
			return v
		}
	}

	return rec.base[row][col]
}
