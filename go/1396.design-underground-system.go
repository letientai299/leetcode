...
type UndergroundSystem struct {
	stores  map[string]map[string][]int // start -> end -> {count, total}
	pending map[int]UndergroundSystemCustomer
}

type UndergroundSystemCustomer struct {
	station string
	time    int
}
...
func (s *UndergroundSystem) CheckIn(id int, start string, t int) {
	s.pending[id] = UndergroundSystemCustomer{
		station: start,
		time:    t,
	}
}

func (s *UndergroundSystem) CheckOut(id int, end string, t int) {
	u := s.pending[id]
	if s.stores[u.station] == nil {
		s.stores[u.station] = make(map[string][]int)
	}

	if len(s.stores[u.station][end]) == 0 {
		s.stores[u.station][end] = append(s.stores[u.station][end], 0, 0)
	}

	s.stores[u.station][end][0]++
	s.stores[u.station][end][1] += t - u.time
	delete(s.pending, id)
}

func (s *UndergroundSystem) GetAverageTime(start string, end string) float64 {
	stat := s.stores[start][end]
	return float64(stat[1]) / float64(stat[0])
}
