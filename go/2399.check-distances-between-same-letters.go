...
// occurrences of the `ith` letter is `distance[i]`. If the `ith` letter does
// not appear in `s`, then `distance[i]` can be **ignored**.
//
// Return `true` _if_ `s` _is a **well-spaced** string, otherwise return_
// `false`.
//
// **Example 1:**
//
