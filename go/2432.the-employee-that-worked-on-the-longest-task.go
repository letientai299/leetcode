...
//
// Return _the id of the employee that worked the task with the longest time._
// If there is a tie between two or more employees, return _the **smallest** id
// among them_.
//
// **Example 1:**
//
...
func hardestWorker(n int, logs [][]int) int {
	...
	for i := 1; i < len(logs); i++ {
		log := logs[i]
		id, leave := log[0], log[1]
		time := leave - logs[i-1][1]
		if time > longestTime || (time == longestTime && id < bestID) {
			bestID = id
			longestTime = time
		}
	}
