...
// users from locking the same node. You may only lock a node if the node is
// unlocked.
// - **Unlock: Unlocks** the given node for the given user. You may only unlock
// a node if it is currently locked by the same user.
// - **Upgrade** **: Locks** the given node for the given user and **unlocks**
// all of its descendants. You may only upgrade a node if **all** 3 conditions
// are true:
//
...
// with id `user` to upgrade the node `num`, or `false` otherwise. If it is
// possible, the node `num` will be **upgraded**.
//
// **Example 1:**
//
...
func Constructor1993(parent []int) LockingTree {
	...
	for c, p := range parent {
		tree.kids[p] = append(tree.kids[p], c)
	}
...
func (lt *LockingTree) Upgrade(num int, user int) bool {
	if lt.locked(num) || !lt.hasLockedChild(num) || lt.hasLockedParent(num) {
		return false
	}
...
func (lt *LockingTree) hasLockedChild(num int) bool {
	for _, k := range lt.kids[num] {
		if lt.locked(k) || lt.hasLockedChild(k) {
			return true
		}
	}
	return false
...
func (lt *LockingTree) locked(num int) bool {
	u, ok := lt.nodes[num]
	return ok && u > 0
}

func (lt *LockingTree) hasLockedParent(num int) bool {
	p := lt.parent[num]
	if p == -1 {
		return false
	}
	return lt.locked(p) || lt.hasLockedParent(p)
}

func (lt *LockingTree) unlockChild(num int) {
	for _, k := range lt.kids[num] {
		delete(lt.nodes, k)
		lt.unlockChild(k)
	}
